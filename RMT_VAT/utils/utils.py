...
def post_process_evaluate(x, target, name, args):
    ...
    for i in range(x.shape[0]):
        ...
        if num == 0:
            ...
            lesion = cv2.resize(lesion, (target_tmp.shape[1], target_tmp.shape[0]), interpolation=cv2.INTER_NEAREST)
...
def multi_validate(valloader, model, criterion, epoch, use_cuda, args):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, name) in enumerate(valloader):
            ...
            # compute output
            # inputs = inputs.permute(0, 3, 1, 2)  # Convert (N, H, W, C) -> (N, C, H, W)
            # print(inputs.shape)  # Should be [16, 3, 964, 1296]
            # print('HI')
            ...
            outputs = model(inputs[:, :, 0:224, 0:224])
            outputs = outputs['out'] if isinstance(outputs, dict) else outputs
            targets = targets.squeeze(1)
            # print("Outputs shape:", outputs.shape)  # Should be [batch, 3, 224, 224]
            # print("Targets shape:", targets[:, 0:224, 0:224].shape)  # Should be [batch, 224, 224]
            ...
            outputs = model(inputs[:, :, 24:248, 24:248])
            outputs = outputs['out'] if isinstance(outputs, dict) else outputs
            ...
            outputs = model(inputs[:, :, 0:224, 24:248])
            outputs = outputs['out'] if isinstance(outputs, dict) else outputs
            ...
            outputs = model(inputs[:, :, 24:248, 0:224])
            outputs = outputs['out'] if isinstance(outputs, dict) else outputs
